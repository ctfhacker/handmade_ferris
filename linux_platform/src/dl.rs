...
pub struct GameFuncs {
    ...
    /// Time stamp of this library to check whether to refresh or not
...
impl GameFuncs {
    ...
    pub fn reload(self) -> Self {
        // If the library hasn't been updated, no need to reload the game functions
    ...
    /// Returns true is the library has been updated
    pub fn is_library_updated(&self) -> bool {
        std::fs::metadata(LIBGAME).unwrap().created().unwrap() != self.time_created
    }
}
...
pub fn get_game_funcs() -> GameFuncs {
    ...
    let metadata = std::fs::metadata(LIBGAME).unwrap();
    ...
    unsafe {
        ...
        GameFuncs {
            ...
            time_created: metadata
                .created()
                .expect("Failed to get libgame creation date"),
        }
    }
}
